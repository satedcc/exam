...
class Jadwal extends CI_Controller
{
    ...
    public function allpartisipant()
    {
        ...
        $data['partisipan'] = $this->m_jadwal->listParticipant($this->input->post('id'))->result();
        $user = $this->session->userdata('akun');
        if ($this->session->userdata('level') != "superadmin") {
            $data["ujian"] = $this->m_ujian->get($user)->result();
        } else {
...
<?php
    ...
    public function detailexam($reg = 0, $jadwal = 0, $token = 0)
    {
        $data['jadwal']     = $this->m_jadwal->getById($jadwal);
        $data['row']        = $this->m_reg->getById($reg);
        $data['jawaban']    = $this->m_jawaban->getNilai($reg, $jadwal);
        $data['nilai']      = $this->m_jawaban->cekNilai($reg, $jadwal, $token);
        $data['need']      = $this->m_jawaban->needAction($token);
        $data['token']      = $token;
    ...
    public function listpartisipan($jadwal)
    {
        $data['partisipan'] = $this->m_jadwal->listParticipant($jadwal)->result();
        $user = $this->session->userdata('akun');
        if ($this->session->userdata('level') != "superadmin") {
            $data["ujian"] = $this->m_ujian->get($user)->result();
        } else {
        ...
        $this->load->view('layouts/header');
        $this->load->view('layouts/sidebar');
        $this->load->view('admin/jadwal/listpartisipan', $data);
        $this->load->view('layouts/footer');
    }
    public function analytics($jadwal)
    {
        $data["analisa"] = $this->m_jadwal->analisa($jadwal);
        $this->load->view('layouts/header');
        $this->load->view('layouts/sidebar');
        $this->load->view('admin/jadwal/analisa', $data);
        $this->load->view('layouts/footer');
    }
}
