...
class Soal extends CI_Controller
{
    ...
    public function exam($id = 0, $ujian = 0, $sid = 0)
    {
        ...
        if ($cek > 0) {
            ...
            $random = ($soal->random == "Y") ? 'RAND()' : 'a.id_detail ASC';
            ...
            $data["temp"]           = $this->m_jawaban->cekTime($sid)->row();
            $this->load->view('layouts/front_header');
    ...
    public function save()
    {
        ...
        $soal['id_soal'] = $this->input->post('soal');
        $soal['jawab'] = $this->input->post('jawab');
        $soal['time'] = $this->input->post('time');
        $soal['jadwal'] = $this->input->post('jadwal');
        $soal['session_id'] = session_id();

        $this->m_jawaban->saveTemp($soal);
    }
    public function jawab()
    {
        ...
        //         $id = $value['jadwal'];
        //     }
        // }$this->session->userdata('akun')

        // $this->session->unset_userdata('jawaban');

        $sid = session_id();
        $temp = $this->m_jawaban->getTemp($sid)->result();
        $token = randomString(40);
        foreach ($temp as $t) {
            $jawab['id_regis']          = $this->session->userdata('akun');
            $jawab['id_jadwal']         = $t->jadwal;
            $jawab['id_soal']           = $t->id_soal;
            $jawab['jawaban']           = $t->jawab;
            $jawab['status_jawaban']    = 'N';
            $jawab['time']              = $t->time;
            $jawab['token_id']          = $token;
            $this->m_jawaban->save($jawab);
            $id = $t->jadwal;
        }
        $datax['status_partisipant'] = "N";
        $this->m_jawaban->disabledJadwal($datax, $this->session->userdata('akun'), $id);
        $this->m_jawaban->deleteTemp($sid);
