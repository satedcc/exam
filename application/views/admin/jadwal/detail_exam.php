                    ...
                    <?php if (isset($jawaban)) : ?>
                        <div class="detail-profile">
                            ...
                            <div class="result-exam">
                                ...
                                <?php if ($need > 0) : ?>
                                    <div class="bg-info rounded-5 mt-2 text-center p-2">
                                        <h5 class="text-white m-0">NEED ACTION</h5>
                                    </div>
                                <?php else : ?>
                        ...
                        <table class="table stripe" id="example1">
                            ...
                            <tbody>
                                <?php
                                $participan = $this->db->query("SELECT * FROM ex_jawaban WHERE id_regis=' $row->id_regis' AND id_jadwal='$jadwal->id_jadwal' AND token_id='$token'")->num_rows();
                                if ($participan > 0) {
                                    $no = 1;
                                    ...
                                    foreach ($soal as $j) {
                                        $jawab = $this->db->query("SELECT * FROM ex_jawaban WHERE id_soal='$j->id_soal' AND id_regis='$row->id_regis' AND id_jadwal='$jadwal->id_jadwal' AND token_id='$token'")->row();
                                        if (isset($jawab)) {
                                            $ans = $jawab->jawaban;
                                        } else {
                                            $ans = "0";
