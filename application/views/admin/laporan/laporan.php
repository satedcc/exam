    ...
    <div class="row">
        <?php if (!empty($peserta)) : ?>
            <?php foreach ($peserta as $p) :
                $jmlsoal    = $this->db->query("SELECT id_regis,id_jawab,id_jadwal FROM ex_jawaban WHERE id_regis='$p->id_regis' AND id_jadwal='$id' AND token_id='$p->token_id'")->num_rows();
                ...
                $nilai      = $this->db->query("SELECT 
                                        ...
                                        FROM ex_jawaban AS a LEFT JOIN ex_soal AS b ON b.id_soal = a.id_soal WHERE a.id_regis='$p->id_regis' AND a.id_jadwal='$id' AND token_id='$p->token_id'")->row();
                ...
                <div class="col-md-3 mb-3">
                    <div class="card card-hover card-task-one">
                        ...
                        <div class="card-body list-user">
                            ...
                            <a href="<?= base_url('admin/jadwal/detailexam/' . $p->id_regis . "/" . $id . "/" . $p->token_id) ?>" class="btn btn-sm btn-primary mt-3 d-block w-100 rounded-5">Detail</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
