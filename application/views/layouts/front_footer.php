...
<script>
    $(document).ready(function() {
        ...
        $("#change-password").click(function(e) {
            if ($(this).is(':checked')) {
                $("#old,#new,#confirm").prop('disabled', false);
                $("#old").focus();
            } else {
                $("#old,#new,#confirm").prop('disabled', true);
            }
...
<?php if (isset($info->jenis_waktu)) : ?>
    <?php if ($info->jenis_waktu == "T") : ?>
        <?php if ($temp->total > 0) : ?>
            <?php $timer_active = $temp->time; ?>
        <?php else : ?>
            <?php $timer_active = $info->timer; ?>
        <?php endif; ?>
        <script>
            $(".countdown").html('<span class="hour">00</span>:<span class="minute">00</span>:<span class="second">00</span>');
            var m = convertH2M("<?php echo $timer_active ?>"); //define your minute
            var s = 0;
            var h;
            var xx = 0;
            var width = 100 / (m * 60);
